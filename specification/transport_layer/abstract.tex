...
UAVCAN supports modular transport redundancy for the benefit of a certain class of safety-critical applications.
A modular redundant transport interconnects nodes belonging to the same network (all or their subset)
...
such that the level of service of the resulting modular redundant transport remains constant
...
interconnected using different concrete transport implementations.
...
\begin{remark}[breakable]
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.6\textwidth]{transport_layer/cyclic_transfer_id_redundant_transport}
...
\subsubsection{Deterministic data loss mitigation}
...
under the same transfer-ID value.
...
in which case implementations shall strive to minimize the temporal skew.

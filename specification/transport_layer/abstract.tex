...
A transfer carries exactly one \emph{serialized object} which is an instance of a well-defined DSDL type
with an associated \emph{transfer metadata}.
A transfer is always represented on the transport layer as one or more \emph{transport frames}.

\subsubsection{Transport frame}

A \emph{transport frame} is an atomic block of data with associated metadata that can be exchanged
over the transport layer network\footnote{%
    For example, a CAN frame is a transport frame of the CAN bus transport layer.
}.
...
\begin{description}
    \item[Message transfer] --- a broadcast transfer that contains a serialized message object and its metadata.

    \item[Service transfer] --- a unicast transfer that contains either a service request object or
    a service response object with related metadata.
\end{description}
...
\begin{description}
    ...
    Single-frame transfers are more efficient in terms of throughput and latency.
    ...
    The payload of a multi-frame transfer is appended with its hash for data integrity check purposes;
    such hash is called \emph{transfer CRC}.
    The algorithm used for computation of the transfer CRC and its representation are defined on a per-transport basis.
\end{description}
...
The transport layer model is illustrated on figure \ref{fig:transport_layer_model}.
A detailed review of each component is provided later.

The model relies on an auxiliary concept of \emph{specifier} -- a collection of identifiers that together
define a category of entities.
Specifiers are ephemeral constructs which are needed only for completeness of the model and for
...
TODO: input session specifier, output session specifier.

\subsubsection{Serialized object}

A transfer carries exactly one serialized representation of an instance of a
specific DSDL type\footnote{Chapter \ref{sec:dsdl}.}.
...
\subsubsection{Transfer priority}\label{sec:transfer_prioritization}
...
\subsubsection{Transfer-ID}\label{sec:transfer_id}
...
An anonymous message transfer is a transfer that can be sent from a node that does not have a node-ID\footnote{%
...
An anonymous message has the same metadata as a regular message, except for the source node-ID.

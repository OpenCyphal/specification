...
\subsubsection{Transport layer model}
...
\begin{figure}[H]
    ...
    \resizebox{\textwidth}{!}{
        ...
        \begin{tabu}{|l l l|X[c,2] X[c] X[c]|l|}\hline\rowfont{\bfseries}
            ...
            Defines the urgency (time sensitivity) of the transferred object.\\\cline{2-7}
            ...
            {\bfseries{} Request} &
            {\bfseries{} Response} &
            Request/response specifier applies to services only.\\\cline{2-7}
            ...
            An integer that uniquely identifies a transfer within its category.\\\hline
        \end{tabu}
    }
...
All nodes in a UAVCAN network should be configured to use the same
maximum amount of data per transport frame\footnote{%
    This property is also known \emph{maximum transmission unit} or MTU.
}.
Failure to follow this rule may render nodes unable to communicate if a transmitting node emits larger frames
than the receiving node is able to process.
...
Certain transport implementations\footnote{Such as CAN FD.} may require a short sequence of zero padding bytes
to be added at the end of the transfer payload due to the low granularity of the frame payload length property;
in that case, the zero padding bytes must be included in the CRC computation as well,
as if they were part of the useful payload\footnote{Non-zero padding is not allowed.}.
...
\begin{remark}
    ...
    UAVCAN v0 shares the same CAN bus with UAVCAN v1 (this version of the standard).
    ...
    \begin{UAVCANSimpleTable}{Protocol version detection based on the toggle bit}{|l l X|}
        ...
        0                   & x             & Keep the state of the toggle bit from the first frame of the transfer
                                              to detect protocol version for multi-frame transfers. \\
    \end{UAVCANSimpleTable}
\end{remark}
...
Additionally, excessive payload data may contain zero padding bytes if required by the particular transport layer.
...
\begin{remark}
    ...
    The requirement that all involved nodes use the same maximum amount of payload data per frame (also known as MTU
    -- the maximum transmission unit) is crucial here.
...
\begin{remark}
    ...
    transport layer implementations shall not discard a transfer even if it is determined that it contains less payload
    data than expected.
\end{remark}

...
The transfer ID computation logic relies on the concept of \emph{transfer descriptor}.
A transfer descriptor is a set of properties that identify a particular set of transfers that originate
from the same node, share the same port ID, and the same kind.
The properties that constitute a transfer descriptor are listed below:
\begin{itemize}
    \item Transfer kind (message publication, service request, or service response).
    \item Port ID (subject ID or service ID).
    \item Source node ID.
...
if the previous transfer of the same descriptor was emitted more than 2 seconds ago.

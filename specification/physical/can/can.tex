\section{UAVCAN/CAN physical layer}
...
The table \ref{table:physical_can_connector_summary} provides an overview of the currently defined connector types
...
\begin{UAVCANSimpleTable}{Standard CAN connector types}{|l l l X|}\label{table:physical_can_connector_summary}
...
An example connector pair is pictured on the figures \ref{fig:physical_can_uavcan_d_sub_connector_device}
and \ref{fig:physical_can_uavcan_d_sub_connector_cable}.
...
(see the figures \ref{fig:physical_can_uavcan_d_sub_connector_device} and
\ref{fig:physical_can_uavcan_d_sub_connector_cable}).
...
The table \ref{table:physical_can_uavcan_d_sub_pinout} documents the pinout specification for the
...
\begin{figure}[hbt]
    \centering
    \includegraphics[width=0.6\textwidth]{physical/can/de-9_connector_male_plug}
    \caption{UAVCAN D-Sub device connector example\label{fig:physical_can_uavcan_d_sub_connector_device}}
\end{figure}

\begin{figure}[hbt]
    \centering
    \includegraphics[width=0.6\textwidth]{physical/can/de-9_cable_female_socket}
    \caption{UAVCAN D-Sub cable connector example\label{fig:physical_can_uavcan_d_sub_connector_cable}}
\end{figure}
...
shown on the figure \ref{fig:physical_can_uavcan_m8_connector_example}.
...
(see the figure \ref{fig:physical_can_uavcan_m8_connector_example}).
...
The table \ref{table:physical_can_uavcan_m8_pinout} documents the pinout specification for the UAVCAN M8 connector type.
...
\begin{figure}[hbt]
    \centering
    \includegraphics[width=0.7\textwidth]{physical/can/m8_connector_pair_female_socket_male_plug}\\
    ...
    \caption{UAVCAN M8 connector pair example\label{fig:physical_can_uavcan_m8_connector_example}}
\end{figure}

\begin{figure}[hbt]
    \centering
    \includegraphics[width=0.7\textwidth]{physical/can/m8_connector_pair_assembled}
...
An example is shown on the figure \ref{fig:physical_can_uavcan_micro_connector_example}.
...
The table \ref{table:physical_can_uavcan_micro_pinout}
documents the pinout specification for the UAVCAN M8 connector type.
...
\begin{figure}[hbt]
    \centering
    \includegraphics[width=0.7\textwidth]{physical/can/jst_gh_connectors}
    \caption{UAVCAN Micro right-angle connectors with a twisted pair patch cable connected
             \label{fig:physical_can_uavcan_micro_connector_example}}
\end{figure}

\begin{figure}[hbt]
    \centering
    \includegraphics[width=0.4\textwidth]{physical/can/jst_gh_termination_plug}
...
The table \ref{table:physical_can_classic_phy_parameters} lists the standard parameters of the CAN PHY for
ISO 11898-2.
...
\begin{UAVCANSimpleTable}{Classic CAN 2.0 physical layer parameters}{|l| X[c] X[c] X[c] X[c]|}%
    \label{table:physical_can_classic_phy_parameters}%
    \textbf{Bit rate [kbit/s]}                              &   1000    &   500     &   250     &   125     \\
    \textbf{Valid range for location of sample point [\%]}  &   75--90  &   85--90  &   85--90  &   85--90  \\
    \textbf{Recommended location of sample point [\%]}      &   87.5    &   87.5    &   87.5    &   87.5    \\
    \textbf{Maximum bus length [m]}                         &   40      &   100     &   250     &   500     \\
    \textbf{Maximum stub length [m]}                        &   0.3     &   0.3     &   0.3     &   0.3     \\
\end{UAVCANSimpleTable}
...
Please refer to the chapter \ref{sec:application} for details.

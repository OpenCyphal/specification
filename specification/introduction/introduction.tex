...
\begin{description}
    ...
    \item[High throughput, low latency, determinism] --- UAVCAN must be suitable for use in
    hard real-time deterministic systems.
    ...
    to extremely resource-constrained microcontrollers.
    The latter imposes severe restrictions on the amount of logic needed to support the protocol.
...
are called ``standard data types'') or by a third party for private use or for public release
(in which case they are said to be ``application-specific'' or ``vendor-specific'',
depending on whether the party is a vendor).

...
the rightmost byte (i.e., the most significant byte) is written into the next byte but the next value in the type
will begin starting with the next bit in that same byte. If there are no further values then the remaining bits
shall be zero (0).

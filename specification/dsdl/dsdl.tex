...
Presedence & Op & A & B & Res & Description \\
...
which the equality operation (equality checked with the \verb|==| operator) is defined for.
If multiple items of the same value is inserted (related to the \verb|==| operator),
...
\begin{itemize}
    ...
    \item \texttt{ \{ 2.2, 3.2 \} } - \verb|==| is not defined for const float.
\end{itemize}

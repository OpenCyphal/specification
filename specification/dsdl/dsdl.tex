...
\subsubsection{The concept of valid encoded representation}

For the purposes of the definition that follows, an \emph{encoded representation of $A$}
is a sequence of serialized data fields joined into a bit string according to the DSDL definition
of the data structure $A$.
The serialization rules are reviewed in detail in the section \ref{sec:dsdl_data_serialization}.

A \emph{valid encoded representation of $A$} is any encoded representation of $A$ that belongs
to the set of all possible encoded representations of $A$.

The example below serves to illustrate the concept.
...
For the provided definition, any sequence of 32 bits is a valid encoded representation of the structure,
since any sequence of 32 bits can be mapped onto a valid unsigned 32-bit integer value.

Similarly, any sequence of 100 bits is a valid encoded representation of the following data structure definition,
because any sequence of 64 bits can be mapped onto a valid IEEE 754 binary64 floating point
representation\footnote{Possibly not-a-number (NaN), which is nevertheless a valid value per IEEE 754.},
and any sequence of 36 bits can be mapped onto a valid signed 36-bit integer value:
...
The examples reviewed above are unconstrained, meaning that any bit sequence is a valid representation
as long as it is sufficiently long.
However, there exist cases where that may not be true.
Specifically, as follows from the definition of data serialization rules,
dynamic arrays and tagged unions impose constraints on the set of acceptable encoded values;
examples of the constraints are listed below:
\begin{itemize}
    \item An encoded array representation cannot contain more items than than the maximum length of the array.

    \item An encoded array length field cannot contain a value that is higher than the maximum length of the array.

    \item An encoded union representation cannot contain more bits than expected by the currently contained alternative.

    \item An encoded union tag field cannot contain a value that is greater than or equal the number of
    alternatives in the tagged union.
\end{itemize}

\subsubsection{Definition of bit compatibility}
...
Or, $A$ is bit-compatible with $B$ if the set of all possible encoded representations of $B$
is a subset of all possible encoded representations of $A$.
...
major version number greater than zero must be mutually semantically compatible with each other.

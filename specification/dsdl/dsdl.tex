...
The service response marker is used by the DSDL compiler to detect whether the current definition is a
message type definition or a service type definition.
...
\begin{itemize}
    \item Field and namespace names should be all-lowercase words separated with underscores,
          and may include numbers, (e.g.: \verb|field_name|, \verb|my_namespace_7|).

    \item Constant names should be all-uppercase words separated with underscores,
          and may include numbers (e.g.: \verb|CONSTANT_NAME|).

    \item Data type names should be in camel case (first letter of all words in the upper case),
          and may include numbers (e.g.: \verb|TypeName|, \verb|TypeName2|).
\end{itemize}

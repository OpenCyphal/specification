...
\subsection{Keywords}\label{sec:dsdl_keywords}
A comprehensive list of non context sensitive keywords and a short description is given below, 
a bold face \texttt{\textbf{X}} is interchangeable with a decimal number of one or more digits (\texttt{[0-9]+}).

\begin{UAVCANSimpleTable}{A comprehensive table of DSDL keywords}{|l l|}
    \textbf{Keyword(s)} & \textbf{Description} \\
    \texttt{saturated}, \texttt{truncated} & Cast mode. \\
    \texttt{offset} & Assertions only: evaluates into the set of possible bit offsets. \\
    \texttt{true}, \texttt{false} & \texttt{bool} variant. \\
    \texttt{bool} & Primitive type. \\
    \texttt{void\textbf{X}} & Primitive type. \\
    \texttt{int\textbf{X}} & Primitive type. \\
    \texttt{uint\textbf{X}} & Primitive type. \\
    \texttt{u\textbf{X}\_\textbf{X}} & Reserved for future use. \\
    \texttt{q\textbf{X}\_\textbf{X}} & Reserved for future use. \\
    \texttt{option}, \texttt{optional}, \texttt{maybe} & Reserved for future use. \\
    \texttt{aligned} & Reserved for future use. \\
    \texttt{const} & Reserved for future use. \\
    \texttt{struct}, \texttt{enum} & Reserved for future use. \\
    \texttt{super}, \texttt{self}, \texttt{root} & Reserved for future use. \\
    \texttt{template} & Reserved for future use. \\
    \texttt{con}, \texttt{prn}, \texttt{aux}, \texttt{nul} & Reserved for future use. \\
    \texttt{com\textbf{X}}, \texttt{lpt\textbf{X}} & Reserved for future use. \\
...
These identifiers are also not allowed to be equal to any of the keywords listed in Section \ref{sec:dsdl_keywords}.

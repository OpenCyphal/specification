...
$D$ is a \emph{structural subtype} of $B$ if, and only if,
$b \leq d$, $B$ is not $D$, neither $B$ nor $D$ define a tagged union, and
...
\begin{remark}
    ...
    A message type that defines zero fields is a structural supertype of any other message type.
    A message type may have an arbitrary number of supertypes as long as at most one of them is non-empty.

    Polymorphic relations are not defined on service types and tagged unions.
\end{remark}

...
As explained above, one definition contains either one or two data structures,
for message types and service types, respectively.

A data structure is an ordered, possibly empty collection of \emph{fields} and/or
unordered, possibly empty collection of \emph{constants}.
The value of a regular data structure is a function of the values of all of its fields.
There is one special case of \emph{tagged unions} --- a tagged union is a special data structure
for which at least two fields are defined, but it is capable of holding exactly one field at any given time.
The value of a tagged union is a function of which field it is holding at the moment and the value of said field.

A field represents a named dynamically assigned value of a statically defined type
that can be exchanged over the network as a member of its containing data structure.
A field can be either of a \emph{primitive type}
(i.e., a type which is defined in this DSDL specification, known to DSDL processing tools a priori,
which need not be defined by the user; for example, an integer)
or a \emph{compound type}
(i.e., a type which is defined in DSDL notation; for example, a standard message type \verb|uavcan.node.Heartbeat|).
Fixed-size and variable-size array types are also available.

A padding field is a special kind of field which is used for data alignment purposes.
Padding fields are not named.

A constant represents a named statically assigned value of a statically defined type
that is provided directly in the DSDL source text by the data type designer.
...
There is a generic term --- \emph{attribute} --- which is used to refer to
either a field (including padding fields) or a constant.

Data type definitions can also contain auxiliary elements, such as deprecation markers
(notifying its users that the data type is no longer recommended for new designs)

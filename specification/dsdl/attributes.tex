...
$$
    ...
    \bigcup\limits_{i=1}^{N_\text{fields}}
    \left\{ \lceil{}\log_2 N_\text{fields}\rceil{} + b : b \in B_i \right\}
$$
where $N_\text{fields}$ is the number of fields defined in the current union definition,
...
$$
    \texttt{\_offset\_} =
    \begin{cases}
        \prod\limits_{i=1}^{N_\text{fields}} B_i        & \quad N_\text{fields} > 0 \\
        \left\{0\right\}                                & \quad N_\text{fields} = 0 \\
    \end{cases}
$$
where $N_\text{fields}$ is the number of fields defined in statements preceding the referring statement
...
\begin{remark}
    \begin{minted}{python}
        ...
        @assert _offset_ == {24}
        uint8[<4] c
        @assert _offset_ == {2 + 24,  2 + 32,  2 + 40,  2 + 48}
    \end{minted}
\end{remark}

...
Polymorphic compatibility is non-commutative.
...
$A$ and $B$ are said to be \emph{mutually semantically compatible} if
$A$ is semantically compatible with $B$ and $B$ is semantically compatible with $A$.
...
\begin{remark}
    ...
    A subscriber expecting $B$ can accept $A$, but the reverse is not true.
    Therefore, $B$ is semantically compatible with $A$; the reverse is false.
\end{remark}
...
\begin{remark}
    ...
    For example, a vendor may publish a data type under their unregulated namespace.
    They would be able to move this type to a public-regulated namespace of a different name
...
\subsection{Port identifier assignment constraints}

% Sigh. Here, we are just saying "do what you think is best, you are on your own".
% We need to work out a rigorous definition of semantic compatibility so that it can be effectively reasoned about
